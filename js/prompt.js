...
// which resolves with the verified user object or null.
// ============================================================
import { S } from './db.js';
import { verify, verifyRole } from './auth.js';
...
export function askPassword({ title = 'Authorise action', reason = '', role = null } = {}){
  return new Promise(resolve => {
    ...
    const roleNote = role
      ? `Only <b>${role}</b> can authorise this.`
      : 'Any active staff password works.';
    ...
    bg.innerHTML = `
      <div class="modal narrow">
        ...
        <div class="modal-body">
          ...
          <label class="field">
            <span>Password</span>
            <input id="apPass" type="password" autocomplete="off" placeholder="Enter your password">
          </label>
          <p id="apErr" class="auth-err" style="color:var(--alert);font-size:12.5px;min-height:18px;margin:0"></p>
        </div>
