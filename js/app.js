...
import { audit, hashPw } from './auth.js';
...
// --- nav ---
const NAV = [
  { key:'pos',       label:'Point of Sale', ic:'🛒' },
  { key:'products',  label:'Products',      ic:'📦' },
  { key:'inventory', label:'Inventory',     ic:'📋' },
  { key:'analytics', label:'Analytics',     ic:'📊' },
  { key:'receipts',  label:'Receipts',      ic:'🧾' },
...
nav.innerHTML = NAV.map(n =>
  `<button data-page="${n.key}"><span>${n.ic}</span>${n.label}</button>`
).join('');
...
function setActor(user){
  document.getElementById('lastUser').textContent = user.name;
  document.getElementById('lastRole').textContent = `Last action · ${user.role}`;
}
  ...
  return new Promise(resolve => {
    const m = modal({
      ...
      body: `
        ...
        <p id="rxErr" class="error" style="color:var(--alert);font-size:12.5px;margin:0"></p>`,
...
async function completeSale(){
  ...
  // Password challenge — any active staff can ring up a sale.
  const actor = await askPassword({
    title: 'Complete sale',
    reason: `Total <b>${peso(t.total)}</b>. Your name will be recorded as the cashier on this receipt.`
  });
  if (!actor) return;
  ...
  const sale = {
    ...
    cashier_id: actor.id, cashier_name: actor.name,
...
VIEWS.receipts = () => {
  ...
  document.getElementById('view-receipts').innerHTML = `
    <div class="card">
      ...
      <table class="tbl">
        <thead><tr><th>Receipt</th><th>Date</th><th>Customer</th><th>Cashier</th><th>Payment</th><th class="num">Total</th><th></th></tr></thead>
        <tbody>${list.length ? list.slice(0, 100).map(s => `
          <tr class="clickable" data-open="${s.id}">
            ...
            <td>${s.status === 'voided' ? '<span class="chip bad">Voided</span>' : '<span class="chip ok">Completed</span>'}</td>
          </tr>`).join('')
          : '<tr><td colspan="7"><div class="empty"><div class="big">No receipts yet</div>Complete a sale to see it here.</div></td></tr>'}
        </tbody>
      </table>
    </div>`;
...
VIEWS.inventory = () => {
  document.getElementById('view-inventory').innerHTML = `
    ...
    <div class="card">
      ...
      <table class="tbl">
        <thead><tr><th>When</th><th>Product</th><th>Type</th><th class="num">Change</th><th class="num">After</th><th>Staff</th></tr></thead>
        <tbody>${(S.movements||[]).slice().sort((a,b) => b.created_at.localeCompare(a.created_at)).slice(0, 30).map(m => {
          const p = (S.products||[]).find(x => x.id === m.product_id);
          return `<tr>
            <td class="tiny">${fmtDT(m.created_at)}</td>
            <td>${esc(p?.name || '—')}</td>
            <td>${esc(m.movement_type)}</td>
            <td class="num" style="color:${m.quantity < 0 ? 'var(--alert)' : 'var(--leaf)'}">${m.quantity > 0 ? '+' : ''}${m.quantity}</td>
            <td class="num">${m.new_stock}</td>
            <td class="tiny">${esc(m.user_name || '—')}</td>
          </tr>`;
        }).join('') || '<tr><td colspan="6"><div class="empty">No movements yet.</div></td></tr>'}</tbody>
      </table>
    </div>`;
};
...
VIEWS.analytics = () => {
  ...
  const todayList = sales.filter(s => s.transaction_date.slice(0,10) === today());
  const todayTotal = todayList.reduce((a,s) => a + Number(s.total), 0);
  ...
  document.getElementById('view-analytics').innerHTML = `
    <div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(180px,1fr));margin-bottom:16px">
      ...
      <div class="stat"><div class="stat-k">Today</div><div class="stat-v">${peso(todayTotal)}</div><div class="stat-d">${todayList.length} transactions</div></div>
    ...
    <div class="card">
      ...
      <table class="tbl">
        ...
        <tbody>${Object.entries(byMethod).sort((a,b) => b[1]-a[1]).map(([k,v]) => `
          <tr><td>${esc(k)}</td><td class="num">${peso(v)}</td><td class="num">${total ? Math.round(v/total*100) : 0}%</td></tr>`).join('')
          || '<tr><td colspan="3"><div class="empty">No sales yet.</div></td></tr>'}</tbody>
      </table>
    </div>`;
};
...
VIEWS.settings = () => {
  ...
  document.getElementById('view-settings').innerHTML = `
    <div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(320px,1fr))">
      <div class="card">
        <div class="card-head"><h3>Pharmacy details</h3></div>
        <div class="card-body">
          <label class="field"><span>Registered name</span><input id="phName" value="${esc(ph.name)}"></label>
          <label class="field"><span>Tagline</span><input id="phTag" value="${esc(ph.tagline)}"></label>
          <label class="field"><span>Address</span><input id="phAddr" value="${esc(ph.address)}"></label>
          <label class="field"><span>Phone</span><input id="phPhone" value="${esc(ph.phone)}"></label>
          <label class="field"><span>TIN</span><input id="phTin" value="${esc(ph.tin)}"></label>
          <button class="btn btn-primary" id="phSave">Save changes</button>
        </div>
      </div>
      <div class="card">
        <div class="card-head"><h3>Data</h3></div>
        <div class="card-body">
          <p class="small muted">Everything lives in this browser's IndexedDB. Export a backup before clearing data.</p>
          <div class="row row-wrap" style="margin-top:12px">
            <button class="btn" id="dExp">Download backup (JSON)</button>
            <button class="btn danger" id="dWipe">Erase everything</button>
          </div>
        </div>
      </div>
      <div class="card">
        ...
        <div class="card-body">
          <p class="small muted">Most recent ${Math.min(20, (S.audit||[]).length)} actions.</p>
          <table class="tbl" style="margin-top:8px">
            <tbody>${(S.audit||[]).slice().sort((a,b) => b.at.localeCompare(a.at)).slice(0, 20).map(a => `
              <tr><td class="tiny">${fmtDT(a.at)}</td><td>${esc(a.user_name)}</td><td>${esc(a.action)}</td></tr>`).join('')
              || '<tr><td><div class="empty">No audit entries yet.</div></td></tr>'}</tbody>
          </table>
        </div>
      </div>
    </div>`;
  ...
  document.getElementById('phSave').onclick = async () => {
    const actor = await askPassword({
      title: 'Save pharmacy settings',
      reason: 'Update pharmacy details on record.'
    });
    if (!actor) return;

    Object.assign(S.settings.pharmacy, {
      name: document.getElementById('phName').value.trim(),
      tagline: document.getElementById('phTag').value.trim(),
      address: document.getElementById('phAddr').value.trim(),
      phone: document.getElementById('phPhone').value.trim(),
      tin: document.getElementById('phTin').value.trim()
    });
    await saveSettings();
    audit('Updated settings', 'Pharmacy details', actor);
    setActor(actor);
    document.querySelector('.sidebar-name').textContent = S.settings.pharmacy.name;
    toast('Settings saved.');
  };

  document.getElementById('dExp').onclick = async () => {
    const actor = await askPassword({
      title: 'Download backup',
      reason: 'Exports every product, sale, receipt and setting as JSON.'
    });
    if (!actor) return;

    const { TABLES } = await import('./db.js');
    const dump = { format:'magallanes-pharmacy-backup', version:1, exported_at:new Date().toISOString(), data:{} };
    for (const t of TABLES) dump.data[t] = await Repo.all(t);
    const blob = new Blob([JSON.stringify(dump, null, 1)], { type:'application/json' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `magallanes-backup-${today()}.json`;
    document.body.appendChild(a);
    a.click();
    setTimeout(() => { URL.revokeObjectURL(a.href); a.remove(); }, 800);
    audit('Exported backup', '', actor);
    setActor(actor);
  ...
  document.getElementById('dWipe').onclick = async () => {
    const actor = await askPassword({
      ...
      role: 'Administrator',
      reason: 'This deletes every product, sale, receipt, movement and setting. Administrator only.'
    });
    if (!actor) return;

    const { TABLES } = await import('./db.js');
    for (const t of TABLES) await Repo.clear(t);
    const { DEFAULT_SETTINGS } = await import('./seed.js');
    const fresh = DEFAULT_SETTINGS();
    fresh.seeded = true;
    await Repo.put('meta', { key:'settings', value:fresh });
    toast('Everything erased. Reloading.');
...
async function staffPanel(){
  ...
  modal({
    ...
    body: `
      <p class="small muted" style="margin-top:0">Passwords authorise privileged actions. To change one, enter a new value and click Save. You'll need an Administrator password to confirm.</p>
      <table class="tbl">
    ...
    onOpen: (root) => {
      root.querySelectorAll('[data-save]').forEach(btn => {
        btn.onclick = async () => {
          ...
          if (!newPw){ toast('Enter a new password first.', 'warn'); return; }
